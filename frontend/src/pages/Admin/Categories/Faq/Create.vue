             ...
             <div class="card mb-4">
                ...
                <div class="card-body">
                    <div class="row mb-3">
                        ...
                        <div class="col-sm-10">
                            <input type="text" v-model="categoryFaq.name"  class="form-control" :disabled="status.sendRequest" :class="{ 'is-invalid': submitted && v$.categoryFaq.name.$error }">
                            <span v-if="v$.categoryFaq.name.$error" class="invalid-feedback"> {{ v$.categoryFaq.name.$errors[0].$message }} </span>
                        </div>
                    </div>
                     <div class="row mb-3">
                        <label  class="col-sm-2 col-form-label">Description</label>
                        <div class="col-sm-10">
                            <textarea class="form-control" v-model="categoryFaq.description" rows="6" :disabled="status.sendRequest"></textarea>
                        </div>
                    </div>
                </div>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...mapActions('categoryFaq', ['create']),
            ...mapActions({
                clearAlert: 'alert/clear' 
            }),
            handleSubmit(e) {
                this.submitted = true;
                this.v$.categoryFaq.$validate()
                if (!this.v$.categoryFaq.$error) {
                    this.create(this.categoryFaq)
                } 
            }
        },
        setup() {
