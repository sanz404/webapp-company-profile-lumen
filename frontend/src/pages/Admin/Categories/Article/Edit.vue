             ...
             <div class="card mb-4">
                ...
                <div class="card-body">
                    <div class="row mb-3">
                        ...
                        <div class="col-sm-10">
                            <input type="text" v-model="categoryArticle.name"  class="form-control" :disabled="status.sendRequest" :class="{ 'is-invalid': submitted && v$.categoryArticle.name.$error }">
                            <span v-if="v$.categoryArticle.name.$error" class="invalid-feedback"> {{ v$.categoryArticle.name.$errors[0].$message }} </span>
                        </div>
                    </div>
                     <div class="row mb-3">
                        <label  class="col-sm-2 col-form-label">Description</label>
                        <div class="col-sm-10">
                            <textarea class="form-control" v-model="categoryArticle.description" rows="6" :disabled="status.sendRequest"></textarea>
                        </div>
                    </div>
                </div>
                <div class="card-footer clearfix">
                    <div class="float-start">
                         <router-link to="/admin/categories/article/list" data-bs-toggle="tooltip" data-bs-placement="top" title="Back To List" class="btn btn-secondary">
...
<script>
    ...
    const SITE_TITLE = "Category Article";
    ...
    export default {
        ...
        setup() {
         ...
         computed: {
            ...mapState('categoryArticle', ['status']),
            ...mapState({
                alert: state => state.alert,
                status: state=> state.categoryArticle.status,
                categoryArticle: state=> state.categoryArticle.data
            })
        },
        created() { 
           this.alert.message = ''
        ...
        methods: {
            ...
            handleSubmit(e) {
                this.submitted = true;
                this.v$.$validate()
                if (!this.v$.categoryArticle.$error) {
                    const { categoryArticle, param } = this;
                    this.updateData({ categoryArticle, param })
                } 
            }
        },
        watch: {
